...
class PlexRepository {
    ...
    suspend fun getMovies(libraryId: String): Result<List<Movie>> = withContext(Dispatchers.IO) {
        try {
            val response = retryWithFallback { service, token ->
    ...
    suspend fun getTvShows(libraryId: String): Result<List<TvShow>> = withContext(Dispatchers.IO) {
        try {
            val response = retryWithFallback { service, token ->
            ...
            for (collection in orderedCollections) {
                ...
                when (collection.id) {
                    ...
                    "recommended" -> {
                        ...
                        if (collection.type == CollectionType.PLEX && collection.id.startsWith("plex_")) {
                            ...
                            android.util.Log.w("PlexRepository", "    ✗ SKIPPED: Not a Plex collection or invalid ID format")
                        }
                    }
                }
            }
    ...
    private suspend fun getPlexCollectionItems(collection: DashboardCollection, selectedLibraries: List<String>): Result<List<ContentItem>> = withContext(Dispatchers.IO) {
        try {
            ...
            // FIX: Always use getCollectionItems (which calls /library/metadata/{id}/children).
            // This works for both Static Collections and Smart/Managed Collections.
            android.util.Log.d("PlexRepository", "  Fetching children for ratingKey: $ratingKey")
            android.util.Log.d("PlexRepository", "  API endpoint: /library/metadata/$ratingKey/children")

            // Use retryWithFallback to automatically try remote connection if local fails
            val response = retryWithFallback { service, token ->
    ...
    private fun buildImageUrl(thumbPath: String?): String? {
        return com.purestream.utils.PlexUrlBuilder.buildImageUrl(thumbPath, currentServerUrl, currentToken)
    }
